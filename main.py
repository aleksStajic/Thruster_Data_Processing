# Display peak values on plots as just a horizontal line
# Display raw voltage plot on graph of overlapped data
...
ind_p.append(0) # graph starts from 0 since phase is 0, so first period starts from index 0
...
# Using ind_p, shift all time data in sync_data such that each period "chunk" starts from 0 
...
# Extract values in DataFrame into individual Numpy arrays for convenience -> these arrays hold the "averaged" data points over 5 periods
v_avg = np.array(df_sorted["voltage"]) # voltage
i_avg = np.array(df_sorted["current"]) # current
T_avg = np.array(df_sorted["force"]) # thrust
t = np.array(df_sorted["time"])
p_avg = np.multiply(v_avg, i_avg)
...
### Determine and store peak values from synced and overlapped data into Pandas dataframe ###
peaks = np.array([np.amax(T_avg), np.amax(i_avg), np.amax(v_avg), np.amax(p_avg)])
troughs = np.array([abs(np.min(T_avg)), abs(np.amin(i_avg)), abs(np.amin(v_avg)), pmin])
...
# Display total thrust vs. current plot
#figc = plt.figure()
...
#plt.scatter(time, voltage, c = "red")
#plt.scatter(time, fitted_voltage, c = "green")
...
with open(my_path + data_folder + "\\stats.txt", mode = 'w') as file_object:
    ...
    print("\nThrust vs. current deadband: ", deadband, file = file_object)
